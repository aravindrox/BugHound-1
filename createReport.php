...
<html>
    ...
    <body>
        ...
        echo '<option value=\"sev2\"> Servere</option>';
        ...
        echo '<option value=\"rep7\" selected> </option>';
        $reportedBy = mysqli_query($con, "SELECT id,name FROM emp");
        while (($row = mysqli_fetch_array($reportedBy)) != NULL) {
            echo "<option value=\"p1\">" . $row['id'] . "  " . $row['name'] . "</option>";
        }
        echo "</select>";
        ...
        echo '<select name="functionalArea">';
        echo '<option value=\"f1\"> UAT</option>';
        echo '<option value=\"f2\"> SIT </option>';
        echo '<option value=\"f3\"> PAT </option>';
        echo '<option value=\"f4\"> Production </option>';
        echo '<option value=\"f5\" selected> </option>';
        echo "</select>";

        echo "<b>&nbsp;Assigned To</b>";
        echo '<select name="assignedTo">';
        $assignedTo = mysqli_query($con, "SELECT id,name FROM emp");
        echo '<option value=\"rep7\" selected> </option>';

        while (($row = mysqli_fetch_array($assignedTo)) != NULL) {
            echo "<option value=\"a1\">" . $row['id'] . "  " . $row['name'] . "</option>";
        }
        echo "</select>";

        echo "<br><br><b><table><tr><td> Comments <br/></b></td>";
        echo '<td><textarea name="comments" rows="2" cols="150"> </textarea></td></tr></table>';
